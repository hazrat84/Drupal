 ...
 */
class BookingResponseController extends ControllerBase {
    
    public function BookingResponsePage() {
        ...
        if($entity->get('is_accepted')->value == NULL){
            $send_mail = true;
        }
            
        if($action == 'accept'){
            $tutorexplaination_form = '';
            $template_header = 'You have accepted the session requested';
            //$entity = \Drupal::entityManager()->loadEntityByUuid('bookinginfo', $booking_id);
            $entity->set('is_accepted', 1);
            $entity->save();
        }else{
            $tutorexplaination_form = \Drupal::formBuilder()->getForm('\Drupal\bookingform\Form\TutorExplainationForm');
            $template_header = 'You are about to decline the session requested';
            $entity = \Drupal::entityManager()->loadEntityByUuid('bookinginfo', $booking_id);
            $entity->set('is_accepted', 0);
            $entity->save();
            // Release the Timeslot
            $timeslot_id = $entity->get('timeslot_id')->value;
            $ts_entity = entity_load('timeslots', $timeslot_id);
            $ts_entity->set('is_booked', 0);
            $ts_entity->save();
        }
        
        if($send_mail){ // if true then send mail to student
            
            if($entity->get('is_accepted')->value == 1){ // Request Accepted
                
                $service_mail_id = 37; // 37 on staging
                    ...
                    $gotopayment_link = $base_url.'/booking/gotopayment?bookingid='.$booking_id;
        
                    $service_mail_body = $service_mail_data['body'][0]['value'];
                    $service_mail_body = str_replace("[gotopayment_link]", $gotopayment_link, $service_mail_body);
                    ...
                    $subject = 'Your Booking has been processed';
                    //echo $service_mail_body;exit;
                    $sent = mail($student_mail, $subject, $service_mail_body, $headers);
                    
                    if($sent){
                        \Drupal::logger('timeslots')->notice('Booking Request Accepted: Your email has been sent '.$student_mail.'.');
                    }else{
                        \Drupal::logger('timeslots')->notice('Booking Request Accepted: There was a problem sending your email '.$student_mail.'.');
                    }
        ...
        $output = array(
            '#theme' => 'bookingresponse_template',
            '#action' => $action,
            '#template_header' => $template_header,
            '#tutor_from' => $tutorexplaination_form,
            /*'#attached' => [
